...
# Haar Cascade classifier for upper body detection and OpenCV KCF tracker
...
# Load the haar cascade upper body detector
print("[INFO] Loading upper body detector...")
...
while True:
    ...
    # Run the upper body detector to find or update upper body position
    if tracker is None or totalFrames % skip_frames == 0:
        print("Detecting")
        # convert the frame to grayscale (haar cascades work with grayscale)
        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)

        # Detect all upper bodies
        upper_bodies = detector.detectMultiScale(gray, scaleFactor=1.05,
                                                 minNeighbors=5, minSize=(30, 30),
                                                 flags=cv2.CASCADE_SCALE_IMAGE)

        # Check to see if an upper body was found
        if len(upper_bodies) > 0:
            # Pick the most prominent upper body
            initBB = upper_bodies[0]

            # start the tracker
            tracker = cv2.legacy_TrackerKCF.create()
            tracker.init(frame, tuple(initBB))
        else:
            print("Upper body not found")
            tracker = None

    # otherwise the tracker is tracking the upper body, update the position
    else:
        # grab the new bounding box coordinates of the upper body
        ...
        if success:
            ...
            # calculate the center of the upper body
            centerX = int(x + (w / 2.0))
            centerY = int(y + (h / 2.0))
